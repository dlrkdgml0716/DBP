...
namespace DBP24
{
    public partial class chatSettingForm : Form
    {
        ...
        public chatSettingForm(int currentUserId)
        {
            ...
            // 메뉴가 열리기 직전에 '고정'인지 '해제'인지 판단해서 하나만 보여주기
            contextMenu.Opening += (s, e) =>
            {
                if (lvChatList.SelectedItems.Count == 0)
                {
                    e.Cancel = true; // 선택된 게 없으면 메뉴 안 띄움
                    return;
                }
    ...
    SELECT 
        ...
        CASE 
            WHEN r.user1_id = @me THEN r.user1_is_pinned
            WHEN r.user2_id = @me THEN r.user2_is_pinned
            ELSE 0
        END AS is_pinned
    FROM ChatRoom r
    ...
    ORDER BY is_pinned DESC, r.updated_at DESC;";
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                // ⭐ [핵심 수정] Tag에 int 대신 클래스 객체를 저장해야 우클릭 메뉴가 인식함
                ...
                if (isPinned) item.BackColor = Color.Ivory; // 고정된 방 색상 표시

                lvChatList.Items.Add(item);
            }
        }

        // ✅ [추가] 핀 상태 변경 DB 업데이트 함수
        private void TogglePinState(bool setPinned)
        {
            ...
            UPDATE ChatRoom
            SET 
                user1_is_pinned = CASE WHEN user1_id = @me THEN @val ELSE user1_is_pinned END,
                user2_is_pinned = CASE WHEN user2_id = @me THEN @val ELSE user2_is_pinned END
            WHERE id = @roomId";

                var parameters = new MySqlParameter[]
                {
            new MySqlParameter("@me", _currentUserId),
            new MySqlParameter("@val", targetVal),
            new MySqlParameter("@roomId", data.RoomId)
                };

                try
                {
                    dbm.NonQuery(sql, parameters);
                    LoadRecentChats(); // 목록 새로고침
                }
                catch (Exception ex)
                {
        ...
        private void lvChatList_DoubleClick(object? sender, EventArgs e)
        {
            ...
            if (item.Tag is ChatRoomTagData data)
            {
                ...
                // 채팅방 나갔다 오면 읽음 처리 등을 위해 목록 갱신
                LoadRecentChats();
            }
            else
            {
                // 여기가 실행되어서 메시지가 뜬 것임 (타입 불일치)
