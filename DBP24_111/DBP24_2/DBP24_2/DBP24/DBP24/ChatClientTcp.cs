                            ...
                            case "IncomingMessage":
                                {
                                    ...
                                    int fromUserId = p.GetProperty("fromUserId").GetInt32();
                                    string text = p.GetProperty("text").GetString() ?? "";

                                    DateTime sentDate;
                                    try
                                    {
                                        sentDate = p.GetProperty("sentDate").GetDateTime();
                                    }
                                    catch
                                    {
                            ...
                            case "OpenRoomResult":
                                {
                                    ...
                                    foreach (var h in historyJson.EnumerateArray())
                                    {
                                        ...
                                        bool isRead = h.TryGetProperty("isRead", out var ir)
                                            ? ir.GetBoolean()
                                            : true; // 기본 true
                                        int chatId = h.GetProperty("chatId").GetInt32();
                            ...
                            case "GetReservedResult":
                                {
                                    ...
                                    OnReservedList?.Invoke(list); // UI에서 사용하도록 이벤트 콜백
                                    break;
                                }
