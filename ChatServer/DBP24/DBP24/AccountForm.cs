...
namespace DBP24
{
    ...
    {
        // 프로필 이미지 파일 경로 (필요하면 사용, 아니면 무시해도 됨)
        private string? _profileImagePath;
        // DB에 넣을 이미지 실제 데이터(byte[])
        ...
        public AccountForm()
        {
            ...
            ZipBox.KeyDown += EnterToRegister;
        ...
        // 주소 검색 버튼
        private void addressSearchBtn_Click(object sender, EventArgs e)
        {
            using (var dlg = new AddressSearchForm())
            {
                if (dlg.ShowDialog(this) == DialogResult.OK)
                {
                    ...
                    ZipBox.Text = dlg.SelectedZoneCode;
                    AddressBox.Text = dlg.SelectedAddress;
                }
            }
        }
        ...
        {
            // ★ ProfileImageHelper를 out byte[] 버전으로 바꿨다는 전제
            _profileImagePath = ProfileImageHelper.ChooseProfileImage(
                this,
                profileBox,
                out _profileImageBytes
            );
        }
        ...
        private async void RegisterBtn_Click(object sender, EventArgs e)
        {
            ...
            var dto = new CreateAccount.AccountInput
            {
                LoginId = id,
                Password = pw,
                RealName = name,                // Users.name
                Nickname = nick,                // Users.nickname
                Zipcode = zipcode,              // Users.zipcode
                Address = addr,                 // Users.address
                DepartmentId = Convert.ToInt32(DeptCombo.SelectedValue),

                // ★ 여기서부터 프로필 이미지 관련
                ProfileImagePath = _profileImagePath,      // 필요하면 경로도 보냄
                ProfileImageBytes = _profileImageBytes     // DB에 BLOB로 넣을 데이터
            };
