...
namespace ChatClientApp
{
    public sealed class LoginManager
    {
        ...
        public async Task<int> RegisterAsync(string loginId, string password, string? nickname = null, string? realName = null)
        {
            ...
            // 스키마에 맞게: login_id, pw, name(실명), nickname, role
        ...
        /// <summary>
        /// login_id로 사용자 한 명 조회 (id, login_id, pw, nickname, role 반환)
        /// </summary>
        public async Task<(int Id, string LoginId, string Pw, string? Nickname, string Role)?> GetUserByLoginIdAsync(string loginId)
        {
            string sql = $@"
SELECT id, login_id, pw, nickname, role
FROM Users
