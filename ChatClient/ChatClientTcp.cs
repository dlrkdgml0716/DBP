...
public class ChatClientTcp
{
    ...
    public async Task ConnectAsync()
    {
        ...
        // Nagle 알고리즘 끄기: 작은 패킷도 즉시 전송 (채팅 반응성 위해)
        _client.NoDelay = true;

        // TCP KeepAlive 활성화: 장시간 유휴 상태에서도 연결 유지 도움
    ...
    private async Task SendAsync(object obj)
    {
        ...
        // 동시에 여러 쓰레드에서 WriteLineAsync가 호출되지 않게 락
        await _sendLock.WaitAsync();
        try { await _w!.WriteLineAsync(json); }
    ...
    {
        // 마지막 로그인 정보 보관 (재접속 로직에서 활용할 여지)
        _lastLoginId = loginId;
        _lastPassword = password;

        // 응답을 비동기로 기다리기 위해 TaskCompletionSource 준비
        ...
        // RecvLoop에서 LoginResult를 받으면 _loginTcs.SetResult 호출
    ...
    //  - text     : 전송할 내용
    //  - 실제로는 서버에 SendMessage 타입의 JSON 전송
    ...
    //  - 구현 상, 서버에서 이 type을 처리해서 DB의 is_read 상태를 업데이트
    public Task MarkReadAsync(int roomId) =>
        SendAsync(new { type = "MarkRead", payload = new { roomId } });

    // 새로 추가: 채팅방 열기 + 히스토리 요청
    //
    //  - peerUserId: 상대방 사용자 ID
    //  - 내부 동작:
    //      1) _openRoomTcs를 새로 생성하여 응답을 비동기로 기다릴 준비
    //      2) 서버에 {type:"OpenRoom", payload:{peerUserId}} 전송
    //      3) RecvLoop에서 type=="OpenRoomResult"를 받으면,
    //         _openRoomTcs.SetResult(OpenRoomResult) 호출
    //      4) 여기서는 그 Task를 await 해서 결과 반환
    ...
    //  - 서버가 연결을 끊거나 오류가 나면 빠져나와 종료dddddd
        ...
        try
        {
            while (_client!.Connected)
            {
                ...
                try
                {
                    ...
                    if (line == null) break; // 서버가 연결 종료한 경우
                }
                ...
                try
                {
                    ...
                    switch (type)
                    {
                        ...
                        //  LoginResult
                        //   서버 → 클라이언트:
                        //   { "type":"LoginResult","ok":true/false,"userId":1? }
                        // ------------------------------------------------
                        case "LoginResult":
                            {
                                ...
                                // LoginAsync에서 기다리던 TaskCompletionSource 완료
                        ...
                        case "IncomingMessage":
                            {
                                ...
                                DateTime createdAt;
                                try { createdAt = p.GetProperty("createdAt").GetDateTime(); }
                                catch
                                {
                                    System.Diagnostics.Debug.WriteLine("[RECV] createdAt parse fail → Now 사용");
                                ...
                                // OnIncoming 구독자가 여러 개일 수 있으므로
                                // 예외가 나더라도 RecvLoop 전체가 죽지 않게 try/catch
                                try { OnIncoming?.Invoke(msg); }
                        ...
                        //        "peerName":"닉네임/이름",
                        //        "history":[
                        //          {"fromUserId":1,"text":"...", "createdAt":"..."},
                        //          ...
                        //        ]
                        //     }
                        //   }
                        //
                        //   → _openRoomTcs.SetResult(OpenRoomResult)
                        // ------------------------------------------------
                        case "OpenRoomResult":
                            {
                                var p = root.GetProperty("payload");
                                ...
                                // OpenRoomAsync에서 기다리던 TaskCompletionSource를 완료
                                _openRoomTcs?.TrySetResult(new OpenRoomResult(roomId, peerName, list));
                                break;
                            }

                        // ------------------------------------------------
                        // 정의되지 않은 type (향후 확장 가능)
                        // ------------------------------------------------
                        default:
